...
class MCSPider(scrapy.Spider):
    ...
    def parse(self, response):
        store_urls = (
            response.css(".store-name.bem-text-body").xpath("./a/@href").extract()
        )
        print("URLS")
        print(store_urls)
        for s in store_urls:
            ...
            yield scrapy.Request(
                s_url, meta={"dont_redirect": True}, callback=self.parse_store
            )

    def parse_store(self, response):

        store_add1 = (
            response.css(".store-information-block").xpath("./div[1]//text()").getall()
        )
