...
class MCSPider(scrapy.Spider):
    ...
    def parse(self, response):
        store_urls = response.xpath(
            '//div[@class="location-container"]/div/a/@href'
        ).extract()
        for s in store_urls:
    ...
    def parse_store(self, response):
        store_name = response.xpath(
            '//*[@id="content"]/div[4]/div/div[1]/h2/text()'
        ).get()
        store_add1 = response.xpath(
            '//div[@id="StoreAddress"]/div[@class="address"]/div[1]/text()'
        ).get()
        store_add2 = response.xpath(
            '//div[@id="StoreAddress"]/div[@class="address"]/div[2]/text()'
        ).get()
        store_city = response.xpath(
            '//div[@id="StoreAddress"]/div/div[3]/span[1]/text()'
        ).get()
        store_state = response.xpath(
            '//div[@id="StoreAddress"]/div/div[3]/span[2]/text()'
        ).get()
        store_zip = response.xpath(
            '//div[@id="StoreAddress"]/div/div[3]/span[3]/text()'
        ).get()
